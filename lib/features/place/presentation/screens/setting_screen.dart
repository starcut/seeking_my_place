...
import 'package:seeking_my_place/features/place/domain/usecases/export_json_use_case.dart';
import 'package:seeking_my_place/features/place/domain/usecases/export_txt_use_case.dart';
import 'package:seeking_my_place/features/place/domain/usecases/import_json_use_case.dart';
...
/// エクスポート（txt / json 形式）は [ExportTxtUseCase] / [ExportJsonUseCase] に接続済み。
...
class _SettingScreenState extends ConsumerState<SettingScreen> {
  ...
  static const List<String> _exportFormatOptions = ['json', 'txt'];
  ...
  /// place_list の全カラムと、紐づく purpose_id を含めた .json ファイルとして
  /// 共有シート (ActionSheet) 経由で書き出す。
  Future<void> _exportAsJson() async {
    await ref.read(exportJsonUseCaseProvider.notifier).execute();
    if (!mounted) return;
    _showExportResultSnackBar(ref.read(exportJsonUseCaseProvider));
  }

  /// 選択した .json ファイルの内容をアプリのデータベースへ取り込む。
  /// place_name・address・url が完全一致する既存 Place がある場合は
  /// 新規登録せず、purposes のみ既存 Place に反映する。
  Future<void> _importJsonFile() async {
    await ref.read(importJsonUseCaseProvider.notifier).execute();
    if (!mounted) return;
    final resultState = ref.read(importJsonUseCaseProvider);
